...
void MoodbarBuilder::AddFramePy(boost::python::list &magnitudes) {
    // convert python list to vector, which can be accessed like a C array
    auto mags = to_std_vector<double>(magnitudes);
    AddFrame(mags.data(), mags.size());
}

boost::python::list MoodbarBuilder::FinishPy(int width) {
    QByteArray ret = Finish(width);

    // copy array values to python list
    // can't use ret.constData() for this due to 0 bytes
    boost::python::list retl;
    for (int i = 0; i < ret.size(); i++) {
        retl.append(ret.at(i));
    }
    return retl;
}
